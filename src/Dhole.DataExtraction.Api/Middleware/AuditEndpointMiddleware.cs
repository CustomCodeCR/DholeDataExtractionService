...
)
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            logger.LogError(exception, "Failed to audit DataExtraction action {Method} {Path}.", context.Request.Method, context.Request.Path.Value);
        }
    }
    ...
    private static string ResolveEventType(HttpContext context)
        => $"data-extraction.http.{ResolveAction(context)}";
    ...
    private static string? ResolveEntityType(HttpContext context)
    {
        ...
        return segments is null || segments.Length == 0 ? null : segments.LastOrDefault();
    }
